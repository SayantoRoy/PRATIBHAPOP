CREATE PROCEDURE [MMS].[MenuItem_bulkupdate]
...
(SELECT p.*, t.update_scope_local_id, t.scope_update_peer_key, t.local_update_peer_timestamp FROM @changeTable p LEFT JOIN [MMS].[MenuItem_tracking] t ON p.[Id] = t.[Id]) as changes ON changes.[Id] = base.[Id]
...
UPDATE SET [MenuId] = changes.[MenuId], [MenuItemGroup] = changes.[MenuItemGroup], [Sequence] = changes.[Sequence], [Code] = changes.[Code], [UserCode] = changes.[UserCode], [InterfaceNo] = changes.[InterfaceNo], [StandardName] = changes.[StandardName], [UserName] = changes.[UserName], [TooltipName] = changes.[TooltipName], [MaximumUser] = changes.[MaximumUser], [MaximumInactiveTime] = changes.[MaximumInactiveTime], [Image] = changes.[Image], [Description] = changes.[Description], [Remarks] = changes.[Remarks], [Active] = changes.[Active], [Archive] = changes.[Archive], [AddedBy] = changes.[AddedBy], [AddedDate] = changes.[AddedDate], [AddedFromIP] = changes.[AddedFromIP], [UpdatedBy] = changes.[UpdatedBy], [UpdatedDate] = changes.[UpdatedDate], [UpdatedFromIP] = changes.[UpdatedFromIP]
