CREATE PROCEDURE [MMS].[MenuAction_bulkupdate]
	@sync_min_timestamp BigInt,
	@sync_scope_local_id Int,
	@changeTable [MMS].[MenuAction_BulkType] READONLY
AS
BEGIN
-- use a temp table to store the list of PKs that successfully got updated
declare @changed TABLE ([Id] varchar(50), PRIMARY KEY ([Id]));

-- update the base table
MERGE [MMS].[MenuAction] AS base USING
-- join done here against the side table to get the local timestamp for concurrency check
(SELECT p.*, t.update_scope_local_id, t.scope_update_peer_key, t.local_update_peer_timestamp FROM @changeTable p LEFT JOIN [MMS].[MenuAction_tracking] t ON p.[Id] = t.[Id]) as changes ON changes.[Id] = base.[Id]
...
UPDATE SET [MenuId] = changes.[MenuId], [Action] = changes.[Action], [UserName] = changes.[UserName], [Description] = changes.[Description], [Active] = changes.[Active], [Archive] = changes.[Archive]
...
FROM 
[MMS].[MenuAction_tracking] side JOIN 
