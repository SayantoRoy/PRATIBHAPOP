CREATE PROCEDURE [MMS].[Menu_bulkupdate]
	@sync_min_timestamp BigInt,
	@sync_scope_local_id Int,
	@changeTable [MMS].[Menu_BulkType] READONLY
AS
BEGIN
...
MERGE [MMS].[Menu] AS base USING
-- join done here against the side table to get the local timestamp for concurrency check
(SELECT p.*, t.update_scope_local_id, t.scope_update_peer_key, t.local_update_peer_timestamp FROM @changeTable p LEFT JOIN [MMS].[Menu_tracking] t ON p.[Id] = t.[Id]) as changes ON changes.[Id] = base.[Id]
...
UPDATE SET [Area] = changes.[Area], [UserName] = changes.[UserName], [Controller] = changes.[Controller], [Href] = changes.[Href], [IFCodes] = changes.[IFCodes], [Description] = changes.[Description], [Active] = changes.[Active], [Archive] = changes.[Archive]
