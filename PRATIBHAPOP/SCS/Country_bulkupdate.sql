CREATE PROCEDURE [SCS].[Country_bulkupdate]
...
(SELECT p.*, t.update_scope_local_id, t.scope_update_peer_key, t.local_update_peer_timestamp FROM @changeTable p LEFT JOIN [SCS].[Country_tracking] t ON p.[Id] = t.[Id]) as changes ON changes.[Id] = base.[Id]
...
UPDATE SET [ContinentId] = changes.[ContinentId], [CurrencyId] = changes.[CurrencyId], [Sequence] = changes.[Sequence], [ICC] = changes.[ICC], [Code] = changes.[Code], [ShortName] = changes.[ShortName], [StandardName] = changes.[StandardName], [UserName] = changes.[UserName], [Nationality] = changes.[Nationality], [GMTMinute] = changes.[GMTMinute], [GMTHour] = changes.[GMTHour], [TINCaption] = changes.[TINCaption], [NIDCaption] = changes.[NIDCaption], [PhoneLength] = changes.[PhoneLength], [NIDLength] = changes.[NIDLength], [TINLength] = changes.[TINLength], [Description] = changes.[Description], [Remarks] = changes.[Remarks], [Active] = changes.[Active], [Archive] = changes.[Archive], [AddedBy] = changes.[AddedBy], [AddedDate] = changes.[AddedDate], [AddedFromIP] = changes.[AddedFromIP], [UpdatedBy] = changes.[UpdatedBy], [UpdatedDate] = changes.[UpdatedDate], [UpdatedFromIP] = changes.[UpdatedFromIP]
