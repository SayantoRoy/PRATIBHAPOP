CREATE PROCEDURE [SCS].[Currency_bulkupdate]
...
(SELECT p.*, t.update_scope_local_id, t.scope_update_peer_key, t.local_update_peer_timestamp FROM @changeTable p LEFT JOIN [SCS].[Currency_tracking] t ON p.[Id] = t.[Id]) as changes ON changes.[Id] = base.[Id]
...
UPDATE SET [Sequence] = changes.[Sequence], [Code] = changes.[Code], [Name] = changes.[Name], [LargeUnit] = changes.[LargeUnit], [SmallUnit] = changes.[SmallUnit], [InWordFormat] = changes.[InWordFormat], [Remarks] = changes.[Remarks], [Description] = changes.[Description], [Active] = changes.[Active], [Archive] = changes.[Archive], [AddedBy] = changes.[AddedBy], [AddedDate] = changes.[AddedDate], [AddedFromIP] = changes.[AddedFromIP], [UpdatedBy] = changes.[UpdatedBy], [UpdatedDate] = changes.[UpdatedDate], [UpdatedFromIP] = changes.[UpdatedFromIP], [Precision] = changes.[Precision]
