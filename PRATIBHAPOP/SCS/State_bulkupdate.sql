CREATE PROCEDURE [SCS].[State_bulkupdate]
	@sync_min_timestamp BigInt,
	@sync_scope_local_id Int,
	@changeTable [SCS].[State_BulkType] READONLY
AS
BEGIN
...
MERGE [SCS].[State] AS base USING
-- join done here against the side table to get the local timestamp for concurrency check
(SELECT p.*, t.update_scope_local_id, t.scope_update_peer_key, t.local_update_peer_timestamp FROM @changeTable p LEFT JOIN [SCS].[State_tracking] t ON p.[Id] = t.[Id]) as changes ON changes.[Id] = base.[Id]
...
UPDATE SET [CountryId] = changes.[CountryId], [Sequence] = changes.[Sequence], [Code] = changes.[Code], [ShortName] = changes.[ShortName], [StandardName] = changes.[StandardName], [UserName] = changes.[UserName], [Description] = changes.[Description], [Remarks] = changes.[Remarks], [Active] = changes.[Active], [Archive] = changes.[Archive], [AddedBy] = changes.[AddedBy], [AddedDate] = changes.[AddedDate], [AddedFromIP] = changes.[AddedFromIP], [UpdatedBy] = changes.[UpdatedBy], [UpdatedDate] = changes.[UpdatedDate], [UpdatedFromIP] = changes.[UpdatedFromIP]
