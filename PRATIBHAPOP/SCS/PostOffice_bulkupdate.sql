CREATE PROCEDURE [SCS].[PostOffice_bulkupdate]
...
(SELECT p.*, t.update_scope_local_id, t.scope_update_peer_key, t.local_update_peer_timestamp FROM @changeTable p LEFT JOIN [SCS].[PostOffice_tracking] t ON p.[Id] = t.[Id]) as changes ON changes.[Id] = base.[Id]
...
UPDATE SET [DistrictId] = changes.[DistrictId], [Sequence] = changes.[Sequence], [Code] = changes.[Code], [ShortName] = changes.[ShortName], [StandardName] = changes.[StandardName], [UserName] = changes.[UserName], [Description] = changes.[Description], [Remarks] = changes.[Remarks], [AddedBy] = changes.[AddedBy], [AddedDate] = changes.[AddedDate], [AddedFromIP] = changes.[AddedFromIP], [UpdatedBy] = changes.[UpdatedBy], [UpdatedDate] = changes.[UpdatedDate], [UpdatedFromIP] = changes.[UpdatedFromIP], [Active] = changes.[Active], [Archive] = changes.[Archive]
